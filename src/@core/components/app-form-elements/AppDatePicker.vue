...
function toLocaleISODate(date) {
  ...
  const timeString = localISOTime.split('T')[1]

  return `${dateString}T${timeString}`
}

const emitModelValue = (dates: Date[]) => {
  console.log('emitModelValue', dates[0])
  console.log('toLocaleISODate', toLocaleISODate(dates[0]))

  emit('update:modelValue', toLocaleISODate(dates[0]))
}
